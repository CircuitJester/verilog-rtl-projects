...
module multi_mshr_cache #(
    ...
    /*
     * ============================================================
     * MSHR OWNER LOOKUP DEBUG
     * ============================================================
     *
     * req_mshr_match_debug:
     *      1 when req_addr is currently owned by an active MSHR.
     *
     * req_mshr_index_debug:
     *      Index of the MSHR owning req_addr.
     *
     * These are combinational observation signals only.
     * They do not modify MSHR state.
     */
    output wire        req_mshr_match_debug,
    output wire [1:0]  req_mshr_index_debug,
    output wire [1:0]  req_class_debug,
...
/*
 * ============================================================
 * DUPLICATE OUTSTANDING REQUEST DETECTION
 * ============================================================
 *
 * A request is considered duplicate when the exact CPU address
 * is already tracked by an active MSHR.
 *
 * Duplicate requests are blocked until the original transaction
 * completes.
 */
...
assign req_mshr_duplicate =
    (mshr_valid[0] &&
     mshr_addr[0] == req_addr) ||

    (mshr_valid[1] &&
     mshr_addr[1] == req_addr) ||

    (mshr_valid[2] &&
     mshr_addr[2] == req_addr) ||

    (mshr_valid[3] &&
     mshr_addr[3] == req_addr);
 ...
 * ============================================================
 *
 * Search all active MSHRs for the requested address.
 *
 * Priority:
 *
 *      MSHR 0
 ...
 *      MSHR 3
 *
 * Under the duplicate-request policy there should normally be
 * at most one matching MSHR.
 *
 * These signals are purely combinational and are intended for
 * verification/debugging.
 */

reg       req_mshr_match_int;
reg [1:0] req_mshr_index_int;

always @(*) begin

    req_mshr_match_int = 1'b0;
    req_mshr_index_int = 2'd0;

    if (mshr_valid[0] &&
        mshr_addr[0] == req_addr) begin

        req_mshr_match_int = 1'b1;
        req_mshr_index_int = 2'd0;
 ...
 *   2'b00 = CACHE HIT
 *   2'b01 = NEW MISS
 *   2'b10 = DUPLICATE OUTSTANDING REQUEST
 *   2'b11 = RESERVED
 *
 * Observation-only logic.
 * This signal does not modify request acceptance,
 * MSHR allocation, refill, or release.
 */

reg [1:0] req_class_int;
...
always @(*) begin
    ...
    else if (req_mshr_duplicate) begin
